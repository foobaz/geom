...
class RandomLineString(shapely.geometry.LineString):
    ...
    def goify(self):
        return 'geom.LineString{' + ', '.join('{%f, %f}' % c for c in self.coords) + '}'
...
class RandomPolygon(shapely.geometry.Polygon):
    ...
    def goify(self):
        ...
        return 'geom.Polygon{' + ', '.join('{' + ', '.join('{%f, %f}' % c for c in ring) + '}' for ring in rings) + '}'
...
class RandomMultiPoint(shapely.geometry.MultiPoint):
    ...
    def goify(self):
        return 'geom.MultiPoint{' + ', '.join(RandomPoint(g.coords[0]).goify() for g in self.geoms) + '}'
...
class RandomMultiLineString(shapely.geometry.MultiLineString):
    ...
    def goify(self):
        return 'geom.MultiLineString{' + ', '.join(RandomLineString(g.coords).goify() for g in self.geoms) + '}'
...
class RandomMultiPolygon(shapely.geometry.MultiPolygon):
    ...
    def goify(self):
        return 'geom.MultiPolygon{' + ', '.join(RandomPolygon([g.exterior] + list(g.interiors)).goify() for g in self.geoms) + '}'
