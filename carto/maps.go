...
func (r *RasterMap) DrawVector(g geom.T, strokeColor,
	...
	case geom.Point:
		p := g.(geom.Point)
		x, y := r.coordinates(p[0], p[1])
		r.GC.ArcTo(x, y, markersize, markersize, 0, 2*math.Pi)
	//case geom.PointZ:
	//case geom.PointM:
	//case geom.PointZM:
	case geom.MultiPoint:
		for _, p := range g.(geom.MultiPoint) {
			x, y := r.coordinates(p[0], p[1])
			r.GC.MoveTo(x, y)
			r.GC.ArcTo(x, y,
...
func (r *RasterMap) coordinates(X, Y float64) (
	x, y float64) {
	x = (X - r.bounds.Min[0]) / r.dx
	y = float64(r.height) - 1. - (Y-r.bounds.Min[1])/r.dy
	return
}
...
func DrawShapes(f io.Writer, strokeColor, fillColor []color.NRGBA,
	...
	m := NewRasterMap(bounds.Max[1], bounds.Min[1],
		bounds.Max[0], bounds.Min[0], 500, f)
